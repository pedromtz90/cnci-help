...
export default function WorkflowsPage() {
  ...
  const load = useCallback(async () => { setLoading(true); try { const r = await fetch('/api/workflows'); const d = await r.json(); setWorkflows(d.workflows || []); } catch (err) { console.error('[workflows] Failed to load workflows:', err); } setLoading(false); }, []);
  useEffect(() => { load(); }, [load]);
  ...
  const handleTest = async (wf: Workflow) => { setTestingId(wf.id); setTestResult(null); try { const r = await fetch('/api/workflows', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'run', workflow: wf.id, data: { question: 'Prueba', studentName: 'Test', studentEmail: 'test@cnci.mx', category: 'soporte', failureCount: 3 } }) }); setTestResult(await r.json()); } catch (err) { console.error('[workflows] Test execution failed:', err); } setTestingId(null); };
